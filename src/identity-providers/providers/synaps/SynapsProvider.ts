...
export class SynapsProvider implements IdentityVerificationProvider {
  ...
  async createVerificationSession(data: Record<string, unknown>): Promise<VerificationSession> {
    const session = await this.client.createSession({ externalUserId: data.externalUserId, type: 'individual', metadata: { firstName: data.firstName, lastName: data.lastName, dob: data.dob, email: data.email, phone: data.phone, address: data.address } }) as Record<string, unknown>;
