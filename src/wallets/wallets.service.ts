...
export class WalletsService {
  ...
  async linkWallet(
    ...
    // Fall back to the base row while migrations 008/009 haven't been applied.
    if (error && error.code === 'PGRST204') {
      console.warn(
        `[wallets] user_wallets is missing the identity columns (migrations 008/009 pending) — ` +
